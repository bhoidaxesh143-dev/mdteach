...
function isCKTestingVersion(version2) {
  if (!version2) {
    return false;
  }
  return ["nightly", "alpha", "internal", "nightly-", "staging"].some((testVersion) => version2.includes(testVersion));
}
function isCKVersion(version2) {
  return isSemanticVersion(version2) || isCKTestingVersion(version2);
}
...
function getLicenseVersionFromEditorVersion(version2) {
  if (isCKTestingVersion(version2)) {
    return 3;
  }
...
function getCKBoxInstallationInfo() {
  const version2 = window.CKBox?.version;
  if (!isSemanticVersion(version2)) {
    return null;
  }
  return {
...
function compareInstalledCKBaseVersion(version2) {
  ...
  return compareSemanticVersions(installedVersion, version2);
}
...
var ckeditor_default = defineComponent({
  name: "CKEditor",
  __name: "ckeditor",
  props: mergeModels({
    editor: {},
  ...
  setup(__props, { expose: __expose, emit: __emit }) {
    ...
    watch(() => props.disabled, (readOnlyMode) => {
      if (readOnlyMode) instance.value.enableReadOnlyMode(VUE_INTEGRATION_READ_ONLY_LOCK_ID);
      else instance.value.disableReadOnlyMode(VUE_INTEGRATION_READ_ONLY_LOCK_ID);
    });
    ...
    function setUpEditorEvents(editor) {
      const emitDebouncedInputEvent = debounce_default((evt) => {
        if (props.disableTwoWayDataBinding) return;
    ...
    onMounted(() => {
      ...
      const supports = getInstalledCKBaseFeatures();
      let editorConfig = appendAllIntegrationPluginsToConfig(Object.assign({}, props.config));
      let prevModelValue = model.value;
      if (model.value) editorConfig = assignInitialDataToEditorConfig(editorConfig, model.value, true);
      (supports.elementConfigAttachment ? Editor.create(assignElementToEditorConfig(Editor, element.value, editorConfig)) : Editor.create(element.value, editorConfig)).then((editor) => {
        instance.value = markRaw(editor);
        setUpEditorEvents(editor);
        if (model.value !== prevModelValue) editor.data.set(model.value);
        if (props.disabled) editor.enableReadOnlyMode(VUE_INTEGRATION_READ_ONLY_LOCK_ID);
        emit("ready", editor);
      }).catch((error) => {
        console.error(error);
      });
...
function useCKEditorCloud(config) {
  return useAsync(() => loadCKEditorCloud(toValue(config)));
}
if (!version || !version.startsWith("3.")) throw new Error("The CKEditor plugin works only with Vue 3+. For more information, please refer to https://ckeditor.com/docs/ckeditor5/latest/builds/guides/integration/frameworks/vuejs-v3.html");
var CkeditorPlugin = { install(app) {
  app.component("Ckeditor", ckeditor_default);
} };
export {
  ckeditor_default as Ckeditor,
  CkeditorPlugin,
  loadCKEditorCloud,
  useCKEditorCloud
};
